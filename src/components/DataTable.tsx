...
export interface Column<T = any> {
  ...
  sortable?: boolean;
  sortValue?: (item: T) => number | string; // Function to get the value for sorting
  className?: string;
...
export const DataTable: FC<DataTableProps> = ({
  ...
  const sortedData = sortColumn ? [...data].sort((a, b) => {
    ...
    // Use sortValue function if provided, otherwise use the key value
    if (column?.sortValue) {
  ...
  return (
    <div className={`bg-white rounded-2xl border border-[#E2E8F080] overflow-hidden ${className}`}>
      ...
      {showPagination && totalPages > 1 && (
        <div
          className="bg-gray-50 md:px-6 md:py-4 py-2 flex flex-col gap-3 md:flex-row items-center justify-between">
          <div className="flex items-center gap-2">
            <button
              onClick={() => handlePageChange(currentPage - 1)}
              disabled={currentPage === 1}
              className="px-1 py-2 text-base font-medium text-gray-500 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <div className="text-base text-gray-700">
            {t('table.results')}: {startIndex + 1}-{Math.min(endIndex, data.length)} / {data.length}
          </div>
        </div>
      )}
    </div>
