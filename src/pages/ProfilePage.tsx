    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-4 md:p-6">
        <div className="flex items-center justify-between">
          ...
          <button className="inline-flex items-center rounded-lg bg-cyan-600 px-3 py-2 text-white text-sm hover:bg-cyan-700">{t('profile.newTest')}</button>
        </div>
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {statCards.map((c) => (
            <article key={c.id} className="rounded-xl overflow-hidden ring-1 ring-gray-200 bg-gradient-to-b from-cyan-600 to-cyan-700 text-white">
              ...
              <div className="p-4">
                <button className="w-full rounded-lg border border-white/30 hover:bg-white/10 py-2 text-sm">{t('profile.scoreDetails')}</button>
              </div>
            </article>
          ))}
        </div>
