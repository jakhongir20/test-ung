/**
 * Generated by orval v7.11.2 🍺
 * Do not edit manually.
 * Respondent Web API
 * 
    API для веб-приложения проведения опросов и тестирования.
    
    ## Основные возможности:
    - Аутентификация по номеру телефона с OTP
    - Управление пользователями и модераторами
    - Создание и прохождение опросов
    - Детальная статистика и отчеты
    - Многоязычная поддержка (узбекский, русский)
    
    ## Аутентификация:
    API использует JWT токены для аутентификации. Получить токен можно через эндпоинты:
    - `/api/auth/send-otp/` - отправка OTP на номер телефона
    - `/api/auth/verify-otp/` - верификация OTP кода  
    - `/api/auth/login/` - вход/регистрация с OTP
    
    После получения токена добавляйте заголовок: `Authorization: Bearer YOUR_ACCESS_TOKEN`
    
 * OpenAPI spec version: 1.0.0
 */
import type { SessionsProgressRetrieve200Progress } from './sessionsProgressRetrieve200Progress';
import type { SessionsProgressRetrieve200QuestionsItem } from './sessionsProgressRetrieve200QuestionsItem';
import type { SessionsProgressRetrieve200Session } from './sessionsProgressRetrieve200Session';

export type SessionsProgressRetrieve200 = {
  /** Общий прогресс сессии */
  progress?: SessionsProgressRetrieve200Progress;
  /** Детальная информация по каждому вопросу */
  questions?: SessionsProgressRetrieve200QuestionsItem[];
  session?: SessionsProgressRetrieve200Session;
};
